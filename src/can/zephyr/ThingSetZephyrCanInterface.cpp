...
#pragma GCC diagnostic ignored "-Wnarrowing"
        int result = isotp_fast_send(&listener->requestResponseContext, txBuffer, len,
...
void ThingSetZephyrCanInterface::AddressDiscoverer::onAddressClaimReceived(const struct device *dev,
                                                                           struct can_frame *frame, void *arg)
{
...
void ThingSetZephyrCanInterface::AddressDiscoverer::onAddressDiscoverSent(const struct device *dev, int error,
                                                                          void *arg)
{
    AddressDiscoverer *self = (AddressDiscoverer *)arg;
    if (error == 0) {
...
bool ThingSetZephyrCanInterface::bind(uint8_t nodeAddress)
{
    if (_nodeAddress == CanID::broadcastAddress) {
        ...
        auto addressClaimId =
            CanID().setMessageType(MessageType::network).setTarget(CanID::broadcastAddress).setBridge(0);
        ...
        int filterId =
            can_add_rx_filter(_canDevice, AddressDiscoverer::onAddressClaimReceived, &claimer, &addressClaimFilter);
        CanFrame txFrame;
        while (1) {
            ...
            int err = can_send(_canDevice, txFrame.getFrame(), K_MSEC(10), AddressDiscoverer::onAddressDiscoverSent,
                               &claimer);
            if (err != 0) {
                k_sleep(K_MSEC(100));
                continue;
            ...
            uint32_t event = k_event_wait(&claimer.events, AddressClaimEvent::alreadyUsed, false, K_MSEC(500));
            if (event & AddressClaimEvent::alreadyUsed) {
                /* try again with new random node_addr between 0x01 and 0xFD */
                claimer.nodeAddress = CanID::minAddress + sys_rand32_get() % (CanID::maxAddress - CanID::minAddress);
                ...
                if (err_cnt_after.tx_err_cnt <= err_cnt_before.tx_err_cnt) {
                    LOG_INF("Using CAN node address 0x%.2X on %s", claimer.nodeAddress, _canDevice->name);
                    break;
                }
...
#pragma GCC diagnostic pop
    ...
    return isotp_fast_bind(&_listener.requestResponseContext, _canDevice, rxAddress, &flowControlOptions,
                           onRequestResponseReceived, &_listener, onRequestResponseError, onRequestResponseSent)
           == 0;
}
