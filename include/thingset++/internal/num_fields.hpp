...
template <class Derived> struct any_base
{
    any_base(std::size_t);
    template <class Base>
        requires(std::is_base_of_v<std::remove_cvref_t<Base>, std::remove_cv_t<Derived>>
                 && !std::is_same_v<std::remove_cvref_t<Base>, std::remove_cv_t<Derived>>)
    constexpr operator Base &() const noexcept;
};
...
template <typename T> struct CountFieldsHelper
{
    ...
    template <std::size_t l, std::size_t nested, std::size_t r> static consteval bool constructible_with_nested()
    {
    ...
    template <std::size_t n = 0> static consteval std::size_t count_max_args_in_agg()
    {
    ...
    template <std::size_t arg_index, std::size_t size = 0> static consteval std::size_t get_nested_base_field_count()
    {
    ...
    template <std::size_t n, std::size_t max_arg_num> static consteval bool has_n_base_param()
    {
    ...
    template <std::size_t max_arg_num, std::size_t index = 0> static consteval std::size_t base_param_num()
    {
    ...
    template <std::size_t index, std::size_t max> static consteval std::size_t constructible_no_brace_elision()
    {
